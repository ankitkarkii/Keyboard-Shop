  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-4 gap-8 my-16 px-7'>
        {product.map((item, i) => {
          const firstImage = Array.isArray(item.image) && item.image.length > 0 ? item.image[0] : '';
          return <Item key={i} id={item._id} name={item.name} image={firstImage} new_price={item.new_price} old_price={item.old_price} quantity={item.quantity} />
        })}
      </div>
    </div>
  )
